...
static const float expStdA = 0.0025f; // 在 expPointA 高度处的标准差 [m]
static const float expPointB = 1.3f;
static const float expStdB = 0.2f;    // 在 expPointB 高度处的标准差 [m]
static float expCoeff;

#define RANGE_OUTLIER_LIMIT 3000 // 量测距离单位为 [mm]
...
int16_t zRangerMeasurementAndRestart()
{
  ...
  uint16_t rangetemp = vl53l0xReadReg16Bit(&dev, VL53L0X_RA_RESULT_RANGE_STATUS + 10);
		...
		i2cdevWriteByte(dev.I2Cx, dev.devAddr, VL53L0X_RA_SYSTEM_INTERRUPT_CLEAR, 0x01);; // 清除中断并开始下一次测量
	}
	else
	{
		return -3;
...
void zRangerTask(void* arg)
{
  ...
  while (1) {
    ...
    // 检查量程是否合理并写入估计器
    // 传感器不应测到 >3 [m]，离群值通常 >8 [m]
    if (range_last < RANGE_OUTLIER_LIMIT) {
      float distance = (float)range_last * 0.001f; // 从 [mm] 换算到 [m]
...
bool zRangerReadRange(zDistance_t* zrange, const uint32_t tick)
{
  ...
  if (isInit) {
    if (range_last != 0 && range_last < RANGE_OUTLIER_LIMIT) {
      zrange->distance = (float)range_last * 0.001f; // 从 [mm] 换算到 [m]
