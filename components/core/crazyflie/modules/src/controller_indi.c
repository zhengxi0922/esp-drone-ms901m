...
static vector_t refOuterINDI;				// 外环 INDI 参考值
static bool outerLoopActive = false ; 		// 为 1 时启用外环 INDI
...
/**
 * @brief 更新 FloatRates 中 p、q、r 的巴特沃斯滤波器
 *
 * @param filter 使用的滤波器数组
 * @param new_values 新值
 */
		...
		const uint32_t tick)
{
	...
	if (RATE_DO_EXECUTE(ATTITUDE_RATE, tick)) {
		...
		if (setpoint->mode.z == modeDisable) {
				// INDI 位置控制器未启用，INDI 姿态控制器为主环
				actuatorThrust = setpoint->thrust;
		} else{
			if (outerLoopActive) {
				// INDI 位置控制器启用，INDI 姿态控制器变为内环
...
//	                                &rateDesired.roll, &rateDesired.pitch, &rateDesired.yaw);
		...
		// 对 roll 和 pitch，若为速度模式则用设定值覆盖 rateDesired，
		// 同时重置 PID 以避免误差累积，否则后续进入水平模式可能不稳定
		...
		float stateAttitudeRatePitch = -radians(sensors->gyro.y); // 适配 Crazyflie 坐标系
		...
		/*
		 * 3 - 对执行器（或 control_t）使用同样的滤波器，使用上一时刻的指令
		 */
		...
		/*
		 * 4 - 计算期望角加速度：
		 * 4.1 - Rate_reference = P * attitude_error，其中姿态误差可用任意算法计算，
		 *       也可使用已有函数如 attitudeControllerCorrectAttitudePID()，
		 *       但在大姿态误差下不够准确，暂时可用。
		...
		/*
		 * 5 - 对每个轴更新：delta_command = 1/control_effectiveness * (angular_acceleration_reference - angular_acceleration)
		 */

		// 角加速度的增量需要控制输入的增量
		// G1 为控制效能。在 yaw 轴上还需要 G2，
		// 它用于补偿螺旋桨转速变化引起的角加速度（惯性较大，详见论文）
		...
		/*
		 * 6 - 将 delta_command 加到指令并限制在允许范围
		 */
	...
	// 推力为零时不累加
	// TODO：这里应有更优雅的方式，否则输入会在起飞前就增加到最大
