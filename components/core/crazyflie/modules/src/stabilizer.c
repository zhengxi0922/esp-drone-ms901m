...
#if USE_ATK_MS901M && ATK_MS901M_USE_MODULE_ORIENTATION
...
#if USE_ATK_MS901M && ATK_MS901M_USE_MODULE_ORIENTATION
static void mapMs901mAttitude(const attitude_t *in, attitude_t *out)
{
  *out = *in;
#if ATK_MS901M_AXIS_SWAP_XY
  float roll = in->roll;
  float pitch = in->pitch;
  out->roll = ATK_MS901M_AXIS_INVERT_X ? -pitch : pitch;
  out->pitch = roll;
  out->yaw = in->yaw;
#else
  out->roll = ATK_MS901M_AXIS_INVERT_X ? -in->roll : in->roll;
  out->pitch = in->pitch;
  out->yaw = in->yaw;
#endif
}

static void overrideStateWithMs901mOrientation(state_t *state)
{
  attitude_t mapped;

#if ATK_MS901M_ORIENTATION_SOURCE == ATK_MS901M_ORIENTATION_SOURCE_ATTITUDE
  attitude_t attitude;
  ...
  mapMs901mAttitude(&attitude, &mapped);
#else
  quaternion_t quat;
  ...
  struct quat q_raw = mkquat(quat.x, quat.y, quat.z, quat.w);
  struct vec rpy = quat2rpy(q_raw);
  attitude_t attitude = {
  ...
  state->attitude = mapped;

  struct vec rpy_mapped = mkvec(radians(mapped.roll), radians(mapped.pitch), radians(mapped.yaw));
  struct quat q_mapped = rpy2quat(rpy_mapped);
  state->attitudeQuaternion.x = q_mapped.x;
  state->attitudeQuaternion.y = q_mapped.y;
  state->attitudeQuaternion.z = q_mapped.z;
  state->attitudeQuaternion.w = q_mapped.w;
  state->attitudeQuaternion.timestamp = mapped.timestamp;
}
#endif
...
#if USE_ATK_MS901M && ATK_MS901M_USE_MODULE_ORIENTATION
      overrideStateWithMs901mOrientation(&state);
#endif
      compressState();
