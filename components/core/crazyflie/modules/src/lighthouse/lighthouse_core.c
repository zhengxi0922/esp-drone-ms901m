...
TESTABLE_STATIC bool getUartFrameRaw(lighthouseUartFrame_t *frame) {
  ...
  // offset 以 6 MHz 时钟表示，转换为时间戳使用的 24 MHz
  frame->data.offset *= 4;
...
// 位置估计方法
// 0 = 在估计器外通过光束交点计算位置。
//     偏航误差在估计器外计算，位置与偏航误差作为预计算值输入估计器。
// 1 = 将扫描角输入估计器。偏航误差在估计器外计算，
//     并作为预计算值输入估计器。
...
/**
 * @brief 识别系统中使用的基站类型。
 * UART 帧数据中没有可直接识别类型的字段，但可以使用统计方法。
 * 对于 V2 基站，beamWord 会变化；而 V1 基站常出现 0x1ffff。
 *
 * @param frame
 * @param state
...
void lighthouseCoreTask(void *param) {
  ...
  while(1) {
    ...
    while((isUartFrameValid = getUartFrameRaw(&frame))) {
      // 如果有同步帧通过，说明只收到了同步帧，没有其他数据，重置状态
