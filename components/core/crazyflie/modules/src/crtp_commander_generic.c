...
static void fullStateDecoder(setpoint_t *setpoint, uint8_t type, const void *data, size_t datalen)
{
  ...
  float quat[4];
  quatdecompress((uint32_t)values->quat, quat);
  setpoint->attitudeQuaternion.q0 = quat[0];
  setpoint->attitudeQuaternion.q1 = quat[1];
  setpoint->attitudeQuaternion.q2 = quat[2];
  setpoint->attitudeQuaternion.q3 = quat[3];
